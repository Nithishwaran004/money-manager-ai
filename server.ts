...
// We must set User-Agent to 'aistudio-build' in httpOptions for telemetry.
...
// Primary AI Parser / Chatbot endpoint
// Local, robust fallback NLP parser to handle cases when Gemini key is exhausted, rate limited, or offline
// Local, robust fallback NLP parser to handle cases when Gemini key is exhausted, rate limited, or offline
function tryLocalParseAndQuery(
  ...
  // Regex to extract numeric amounts (supporting optional currency prefixes like ₹, Rs, etc.)
  ...
  // If we have a transaction proposal (numbers are present)
...
// Audio transcription endpoint using Gemini AI - Android optimized
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // Build context about the user's current accounts and transactions
...
You must reply with a valid JSON matches the following schema:
{
  "action": "parse" | "query" | "general",
  "parsedTransaction": {
    "type": "income" | "expense" | "transfer",
    "amount": number,
    "category": string (e.g. "Food", "Transport", "Ref", "Salary"),
    "account": string (source account name),
    "toAccount": string (destination account name, only if type is "transfer"),
    "note": string (short description of the spend),
    "date": "YYYY-MM-DD"
  },
  "answerText": "Your direct response explaining the query answer or polite chat reply in the corresponding user language (English or Tamil)."
}
`;

    // Basic text tasks call
    const response = await ai.models.generateContent({
    ...
    console.warn("Gemini processing error, falling back to local NLP parser:", error);
    try {
      const fallbackResult = tryLocalParseAndQuery(req.body.message, req.body.transactions, req.body.accounts, req.body.categories, req.body.locale);
...
// Setup server and live development reloading/production assets serving
async function bootstrapServer() {
  if (process.env.NODE_ENV !== "production") {
    ...
    // Serves compiled resources in production
